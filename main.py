    ...
    try:
        ...
        for container in product_containers:
            ...
            if preco_promo_tag and preco_antigo_tag and desconto_tag:
                # Remove qualquer "R" ou "R " e formata com R$ fixo
                preco_promo = preco_promo_tag.text.strip().replace('R', '').replace('R ', '').replace(',', '.').strip()
                preco_antigo = preco_antigo_tag.text.strip().replace('R', '').replace('R ', '').replace(',', '.').strip()
                desconto = desconto_tag.text.strip()
                preco_text = f"R$ {preco_promo} (R$ {preco_antigo} {desconto})"
            elif preco_promo_tag:
                preco_text = f"R$ {preco_promo_tag.text.strip().replace('R', '').replace('R ', '').replace(',', '.').strip()}"
            else:
                # Verifica preço normal
                preco_normal_tag = container.find('span', class_='sc-fLlhyt fKrYQk sc-14455254-0 sc-c5cd0085-9 ezNOEq dDNfcV')
                if preco_normal_tag:
                    preco_text = f"R$ {preco_normal_tag.text.strip().replace('R', '').replace('R ', '').replace(',', '.').strip()}"
                else:
                    ...
                    if preco_container:
                        ...
                        preco_text = f"R$ {preco_fallback_tag.text.strip().replace('R', '').replace('R ', '').replace(',', '.').strip()}" if preco_fallback_tag else "Preço não encontrado"
                    else:
